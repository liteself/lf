...
if __name__ == '__main__':
    ...
    for filename in lists:
        ...
        knMax = x.groupby(["segments"])["kN"].max()
        knMin = x.groupby(["segments"])["kN"].min()


        mmMax = x.groupby(["segments"])["mm"].max()
        mmMin = x.groupby(["segments"])["mm"].min()
        
        result = (knMax - knMin) / (mmMax - mmMin)
